...
TextModel.prototype.restoreSelection = function(el) {
  ...
  dom.setSelection(el, sel[0], sel[1]);
};
...
ViewModel.prototype.wrapText = function(startOffset, endOffset, nodeOrDesc) {
  ...
  if (start.node != end.node) {
    // Check if the end offset is actually after `end`, e.g.:
    // if (end.node.textContent.length == end.offset)
  ...
  dom.wrapNode(nodeToWrap, dom.createElement(doc, nodeOrDesc));
};
